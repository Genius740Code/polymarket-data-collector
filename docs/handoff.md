...
> ## 🔁 LOOP STATUS (2026-09-12 ~17:00 UTC — VPS: KAGGLE STALL FIXED+VERIFYING, 4/4 LANES UPLOADING, OOM STORMS MANAGED — UNPUSHED `4bb0da7,2112c24,5b9a993,d05b00f,e831c0e,25eef2f`)
> **Box:** `/home/fese/polymarket-data-collector` (VPS, 3.9GB RAM / 19GB disk, 60% disk). **Session:** opencode via Muse Spark; kernel OOM killed this session 2× (949MB) — **keep ALL tool outputs SMALL** (counts/tails only, never full tables/logs; progress-bar lines in logs are MBs long and will truncate/kill your context).
> **Original user bug ("collector ran long but nothing uploaded to Kaggle"):** FIXED + live-verified. Root causes (44h stall since Sep-9 20:05 UTC): (1) kaggle loop did all 4 lanes/tick (~40min/lane) > process lifetime → never reached upload → now ONE lane/tick, persistent round-robin cursor `data/kaggle_staging/_lane_cursor.json`; (2) Step-1b validation re-read the whole hive → now metadata-only worker manifests; (3) pyarrow25 `FileMetaData.schema` leak (~250KB/call, ~1GB/pass) in footer ordering → now date/mtime ordering; (4) ParquetWriter 1M-row groups buffered whole files till close (~1GB spike) → coalesced ~20k RGs (also compaction + rechunked 229 giant files in place); (5) trades full-concat+enrichment killed workers → streaming trades driver (cached pools, global have-set reconcile, narrow writeback; parity-proven vs legacy); (6) Step-1b/retention prune used max-checkpoint → first catch-up prune DELETED 15m/1h/4h rows never uploaded (killed mid-run; loss confined, see gaps) → min-upload gating + per-(lane,dataset) staging-freshness coverage proof + mtime/projection fast scan; (7) WAL replay aborted on one truncated line (73MB stuck forever, rows lost) → skips bad lines, self-heals; (8) trades enrichment unbounded → 420s deadline + sliced fallbacks; (9) tick slept full interval after completion → sleep accounts for tick duration.
> **Commits (NOT PUSHED, do not push unless operator says):** `4bb0da7` stall fix (pytest 177) · `2112c24` prune-scan OOM (fast prune) · `5b9a993` streaming trades + WAL + cadence (181) · `d05b00f` min-gating + floor 550 (184) · `e831c0e` per-dataset coverage proof (184) · `25eef2f` vectorized summary ticks + pm2 cap 1500M (185, pytest **185/185**). `git status` MUST show only src+tests+ecosystem+DATA_CARD — a foreign TS scaffold (`apps/ services/ packages/ infrastructure/ openapi/ *.json`, +4 lines in `.gitignore`) appears/disappears in this dir from another session: NEVER `git add -A`, NEVER delete it, commit only explicit paths.
> **Live state:** collector online (round-robin 5m→15m→1h→4h, all 4 datasets `ready`, 39/39 files remote-verified). Last uploads — 5m: Sep-11 18:38 (manual catch-up, COMPLETE span) then tick 01:03 died OOM 02:30 + tick 11:13 uploaded 13:23 (verify!) · 15m: Sep-12 04:44 + 20:35 · 1h: 22:20 + Sep-12 07:11 · 4h: Sep-12 00:39 + 11:09. Backfill cron STOPPED by hand during OOM squeeze (resolutions via in-run loop meanwhile) — RE-ENABLE (`pm2 start polymarket-resolution-backfill`) when stable. Watchdog online but crash-looped 1931× historically. pm2 `max_memory_restart` edit (2048M→1500M) is COMMITTED but NOT RELOADED (needs `pm2 reload`, restarts collector — do between ticks).
> **Data health (honest):** 5m COMPLETE. Documented gap (DATA_CARD): 15m/1h/4h `book_snapshots(_clean)` Sep-9 20:0x→Sep-11 06:39 (prune max-checkpoint bug, unrecoverable — no history API). events/trades/chainlink/markets have NO hive loss (killed prunes in time); 15m/1h/4h trades+chainlink staging was stale-prior but rebuilds fresh now (streaming+floor550; 4h fully fresh 11:09). BTC WS churn ~10/h exchange-side closes, 100% resync-healed (429/429). Coverage gaps even across assets (restarts/maintenance only). Rollup: `book_snapshots_500ms` 2.2G + events ~1G + trades ~0.5G + staging ~4G ≈ data/ ~5G after safe prune deleted 88k files/5.6M rows (5m tick).
> **OOM reality (read before acting):** box fits ~parent 1.1GB + worker 0.6GB + opencode 0.65GB + system. Kernel OOMs so far: Sep-11 20:56 (parent 2.1GB, prune full-scan — fixed), Sep-12 02:30 (parent 1.18GB + trades worker 0.6GB + session 0.82GB + backfill), Sep-12 14:59 (parent 1.24GB), Sep-12 15:39–15:58 (4× ~1GB workers + session 949MB). Mitigations live: worker 700MB self-cap, floor 550 pre-flight gate, fail-closed keeps-prior everywhere, fast prune. Parent sits ~1.1–1.2GB steady (mimalloc retention; growth flat, not leaking fast — [mem] lists/PosixPath worth a leak hunt later). Worker cap thread samples every 2s — a fast 700→1000MB spike can beat it (the 15:39 pattern); the trades driver peak (~600MB) is uncomfortably close to the cap on big lanes.
> **Rules learned the hard way:** NEVER edit `src/` mid-tick (workers import fresh code per spawn → parent/worker version skew fails manifests; benign but wastes a tick) — stage code only between ticks or accept one wasted tick. NEVER run heavy in-session python (full-hive reads/concats OOM the box; metadata-only + tiny fixtures). Stopping collector/backfill mid-export is safe (fail-closed, WAL-replayed, honest coverage_gap) — prefer controlled stops over letting the kernel kill. `pm2 stop` persists for cron apps; `pm2 start` re-arms.
> **NEXT (in order):** 1) confirm current 4h tick uploads + 5m tick after it (5m stale since 18:38+16h — overdue); 2) `pm2 reload ecosystem.config.js --only polymarket-collector` between ticks (1500M cap + fresh 150MB parent); re-enable backfill; 3) sleep hours, recheck (uploads advanced all lanes, restarts static, no OOM in journal, disk <70%, staging row counts sane); 4) follow-ups (NOT tonight): trades driver peak headroom, parent Path/list retention hunt, compaction for 71k+ tiny chainlink files (summary chainlink loop + every full-hive scan degrades daily), incremental markets_summary (6000+ markets × per-market nearest-tick rescans every tick), lane-lag watchdog alert (would have caught the original 44h stall), sleep-interval already fixed.
>

> ## 🔁 LOOP STATUS (2026-09-08 ~17:50 UTC — WINDOWS DEV BOX: P0 LEAK HUNT SESSION 2 FIXED + COMMITTED `5aa2feb`, NOT PUSHED — VPS VERIFY NEXT)
> **What was done here (this box is the Windows dev checkout, NOT the VPS — no pm2, no .venv-Linux, no /tmp/opencode):**
> 1. Pulled to `367eae8`, built fresh `.venv` (Windows, Python 3.13.14, `pip install -e ".[dev,kaggle]"`), pytest baseline **123/123**.
> 2. Wrote `leak_probe.py` (repo root, committed): counter probe on the plain prod path (config/collector.yaml, 7 assets, 5m-only) + external RSS sampling. Ran it live 13.6 min. **RESULT: local path FLAT — RSS 247→303MB incl. tracemalloc overhead (~2MB/min drift vs ~110MB/min on prod). Leak #2 does NOT reproduce locally in the clean case.** Logs: `logs/leak_probe_session2.log`, `logs/rss_external_20448.log`. All suspect counters flat or bounded: books/markets 14→35 (5m rollover, 6h-evicted), resync episodes/buffers **0** (planned 150s recycles create no episodes), chainlink_events +~415/min (capped 20k).
> 3. Static audit found every container that IS unbounded under prod-only conditions (real disconnects/escalations/long uptime) and **fixed all of them** (commit `5aa2feb`, tests **131/131** = 123 + 8 new in `tests/test_leak2_regression.py`):
>    - **resync.py**: `_episodes` never evicted → FIFO eviction of FINISHED episodes at `MAX_EPISODES=500` (open never evicted; runs after insert so len ≤ cap). `_buffers` → hard cap `MAX_BUFFERED_MSGS_PER_EPISODE=50000` with honest drop counter + throttled `book_anomaly{reason:resync_buffer_overflow}` events (never silent). **Escalation path (most likely leak #2): `resync()` escalation returned False leaving the episode open + its buffer alive — `_replay_buffer_id` kept feeding EVERY live WS msg for the asset into a never-consumed deque. Now: buffer popped + episode marked `_escalated`.** Replay loop skips non-dict markers (old `buffer_message(rid, None)` marker would have crashed `.get`).
>    - **collector.py**: `_replay_buffer_id` only returns ids whose buffer still exists (escalated episodes can't re-arm). Chainlink RAM store append extracted to `_note_chainlink_event` with `CHAINLINK_RAM_CAP=20000` (the old inline 20k trim existed at only ONE of two append sites). 6h ended-market eviction extracted to `_memory_eviction_tick(now_ms)`: now also drops that market's PERSISTED episodes/buffers and prunes final+persisted `_episode_latest`/`_episode_persisted` entries (unpersisted episodes NEVER dropped — data-loss guard, tested).
>    - **markets_log.py**: `_seen_condition_ids` (~2k cids/day, was unbounded) FIFO-capped at `MAX_SEEN_CONDITION_IDS=20000` with `_seen_order`; recent-cid duplicate suppression verified by test.
> 4. Also verified already-bounded: parquet_writer dedup (100k/dataset cap), `books/markets` 6h eviction (pre-existing), `_kaggle_uploads` (200 cap), rollover states (current+next per lane), `markets_log._staging` (flushed every 30s), `_chainlink_events` startup seed (30min cutoff + now capped).
> **What the next AI must do (IN ORDER) — all on the Linux VPS (`/home/fese/polymarket-collector`, `.venv/bin/python` ONLY, `df -h /` gate 150M, check `uptime` if runs die silently):**
> 1. `git pull origin master` → expect `5aa2feb` on top (this box did NOT push — push or pull via whatever channel is agreed; if push is forbidden, cherry-pick `5aa2feb`).
> 2. `.venv/bin/python -m pytest tests/ -q` → expect **131 passed**.
> 3. **Reproduce-with-counters on the VPS**: `pm2 stop polymarket-collector`, run `.venv/bin/python leak_probe.py --duration-sec 780 --sample-sec 60 2>&1 | tee logs/leak_probe_vps_s3.log`, sample RSS externally (`ps -o rss= -p <pid>` every 30s). Compare `=== T+` deltas. Expect flat. If it still grows on the VPS but not locally, the difference is env (Linux asyncio/urllib vs Windows, real disconnect churn, disk pressure) — get `py-spy dump --pid` on the growing process and diff counters vs the local flat run in `logs/leak_probe_session2.log`.
> 4. **15+ min plain CLI flat-RSS verify** (prod config, RSS sampled from outside, growth ≈ 0), then `pm2 start ecosystem.config.js` (FULL-FILE restart so the 1536M cap applies — restart-by-name keeps stale caps) + `pm2 save`.
> 5. Only then: re-enable 15m (config `timeframes` + soak ≥1h + `series_id == "{ASSET}-15m"` purity assert on all 7 lanes), then 4h; re-arm backfill cron (or document the pm2 cron 1G hardcode workaround); `markets_log.flush_staging` retry hardening (drops writer-refused rows — task0 notes). Commit each step. Write `TEST_RUN_REPORT_<date>_MULTI_TF.md` addendum with numbers (RSS readings+timestamps, counter deltas, pytest counts, Kaggle remote-verified rows).
> **Standing rules (unchanged):** real data only; no manual deletes of `data/`; gaps stay honest; the 1.6G `data/raw_ws_archive/` purge needs operator OK; 1354 uncompacted flush files per 2.5h tax every rglob (compact cron off by design — do NOT enable unasked); Do NOT push from dev boxes unless operator says so.

> ## 🔁 LOOP STATUS (2026-09-07 ~22:20 UTC — VPS: ALL TASKS 1–6 DONE, prod on 5m rollout step 1)
> Tasks 2/3/4/5 PASS (see `TEST_RUN_REPORT_2026-09-07_MULTI_TF.md`, committed `dae8b8d`).
> Task 6: pm2 prod up; 21-lane set doesn't fit box (RSS ~110MB/min → P0 leak hunt OPEN; raw archive ~1.8GB/h → disabled on this box); running 5m-only per RUNBOOK §7 step 1 with cron uploads verified 3×. Prod LEFT RUNNING (collector+watchdog+backfill). NEXT: P0 leak hunt, then 15m→4h rollout soaks.
> **Task 4 15m lane PASS** (`run_2x5min_test.py --timeframe 15m`, log `test_run_20260907_multiTF_15m_v5.log`): exit 0, 14 windows (2×15m × 7 assets), **completeness 100.0% / clean 99.92%**, coverage_gaps 0 (all gap counters 0), critical_null false, staging 39/39 remote-verified on `gghgg1/polymarket-15m-crypto`. Staging filter: 7/7 assets pure `series_id == "{ASSET}-15m"` (3614 rows each, zero cross-TF). Collector events full: initial_discovery 7, market_added 28, rollover 21/21, no coverage_gap/subscription_failed.
> **Infra notes:** attempts 1–2 killed by host reboots (16:29, ~17:40 UTC, box `up` reset — not code). Attempt 3 reached 1440s/1890s then **earlyoom SIGKILLed the collector (exit -9, pid 3150, syslog)** during the 2nd interim chunk upload — pandas export of the growing hive overlapping collection peaks RAM on this 3.9G box. Attempt 4 set `test_upload_interval_seconds: 3600` (no interim uploads; final upload after collector exit) → clean pass; yaml reverted to 600 after (prod uses `upload_interval_seconds`, unaffected either way). Partial attempt-3 evidence still valid: 15m discovery/staging/backfill all worked; its only defect was the cut-short collection.
> **skip_datasets fix validated end-to-end:** published 15m `collector_events.parquet` = 535 rows full history (vs 2-row truncation in the 5m run's latest version — superseded artifact, history versions retain data).
> **Task 5 prune dry-run PASS:** `cleanup_local_data(..., rolling_window=True, retention_hours=48, dry_run=True)` on 744M real 15m data lists nothing (all fresh); `data/` untouched; old-file deletion path covered by `test_tf_filter_and_rolling_prune`.
> Caveat: final backfill deferred trades second-pass enrichment (newest trade 622s old <900s heal threshold; "15-min cron will pick it up") — published 15m trades carry first-pass enrichment only.
> NEXT: Task 6 pm2 prod + `TEST_RUN_REPORT_<date>_MULTI_TF.md`; optional markets_log staging-drop hardening.

> ## 🔁 LOOP STATUS (2026-09-07 ~11:30 UTC — VPS session: Task 2 static + Task 3 5m PASS, found+fixed test-prune event truncation)
> **Task 2 static:** pytest 120/120 (with `.venv`; system python3 is shadowed by sibling `polymarket-data-collector` editable install — use `.venv/bin/python`); probe re-run: **5m/15m/4h ENABLE 7/7, 1h/1d OFF**; `Collector(cfg)` constructs, 21 lanes, window sizes match yaml.
> **Task 3 5m live PASS** (`run_2x5min_test.py`, log `test_run_20260907_105741.log`): exit 0, `[test-mode:real] lane restricted to 5m`, 14 windows discovered, **completeness 100.0% / clean 99.95%**, coverage_gaps 0, rollover_misses 0, write_failures 0, critical_null false, staging 39/39, Kaggle `gghgg1/polymarket-5m-crypto` ready + remote-verified (BTC snapshots 1359 rows, all `BTC-5m`). Kaggle dataset-delete 403 at test start (tolerated, by design).
> **Found + fixed (test harness, NOT collector):** interim t10min chunk upload published full event files (229 collector_events rows), but the test-buffer prune (`retention_hours=0`, mtime fallback for TS datasets) deleted `collector_events` (11 files) + `chainlink_events` (28 files) BEFORE the final backfill re-upload rebuilt staging from scratch → published latest version had collector_events 229→2 rows, chainlink ~765→11 rows (market snapshots/books/trades intact). Fix: `cleanup_local_data(..., skip_datasets=...)` (default None, prod behavior unchanged) + test-buffer prune passes `("chainlink_events","collector_events","resync_episodes")`; regression test `test_prune_skip_datasets_keeps_event_history`. pytest **121/121**. Validated live by Task 4 next.
> NEXT: (1) Task 4 `python run_2x5min_test.py --timeframe 15m` (~40 min) + `series_id == "BTC-15m"` staging assert + confirm published collector_events/chainlink full; (2) Task 5 prune dry-run gate; (3) Task 6 pm2 + report; (4) optional markets_log staging-drop hardening.

> ## 🔁 LOOP STATUS (2026-09-07 later — session-3: TASK 0 SOLVED, fix committed+pushed; tasks 3–6 still pending, need the Linux VPS)
> **Missing-events anomaly root-caused and fixed.** Root cause: in `_run_asset_loop` (collector.py) the `_discovery_poller` task was created INSIDE the post-connect `async with websockets.connect(...)` body. During a total outage `connect()` raises before that body ever runs, so `check_and_roll_all` was NEVER called while WS was down — zero `discovery_timeout`/`initial_discovery`/`discovery_poll` rows even though the reconnect loop visibly cycled (`ws_reconnect_attempt`/`book_anomaly` flowed fine). Hypotheses: (a) loop-starvation ELIMINATED, (b) emit-path drop ELIMINATED (writer appends fine; backpressure WAL-spills and returns True), (c) early-return ELIMINATED — proven by a full-path repro, not isolation tests.
> **Repro (now a regression test):** `tests/test_outage_discovery_events.py` — every endpoint pointed at `https://nonexistent.invalid` (RFC 2606), full `Collector.start()` for ~25s, assert `discovery_timeout` AND `initial_discovery` rows exist in `data/collector_events/` parquet. On 97d6e07 it FAILS with `by_type={'book_anomaly': 14, 'ws_reconnect_attempt': 10, 'scheduler_lag': 1, 'snapshot_heartbeat': 2}` — exactly the overnight signature. With the fix it passes.
> **Fix (collector.py):** hoisted `_discovery_poller` + `_on_market` + `_ensure_ws_subscription` ABOVE the connect loop so discovery runs for the whole asset-loop lifetime, WS up OR down; the task is registered in `self._tasks` (survives 150s recycles and every reconnect; cancelled only at stop). A `ws_holder` dict publishes the live socket; the per-connection body resets its subscribe state; the per-connection `finally` clears `ws_holder["ws"]` (no sends to dead sockets) and cancels only heartbeat/watchdog. Static: **pytest 120/120** (119 + 1 new).
> **Secondary finding (NOT yet fixed — next session):** `MarketsLog.append_event`/`append` keep writer-refused rows (`writer.append()` returned False: WAL disabled or WAL write failed, e.g. disk-full) in `_staging` "for retry", but `flush_staging()` with a writer just CLEARS the list — those rows are silently dropped. Under the 97%-disk night this was a plausible secondary loss path for events; fix = real retry in flush_staging (re-append refused rows, bounded) + regression test.
> **Environment:** this session ran on a FRESH WINDOWS box (Win10, Python 3.14.5, repo venv `.venv\`, git via winget; clone of master @ 97d6e07). No kaggle.json, no pm2/node, no local hive data → Tasks 3/4 (destructive 5m/15m live runs), 5 (prune dry-run needs real local data) and 6 (pm2 prod) CANNOT run here — they need the Linux VPS with Kaggle creds. Nothing was uploaded/deleted.
> NEXT: (1) on the VPS: re-run Task 3 `python run_2x5min_test.py` (gate: exit 0, completeness ≥99%, coverage_gaps 0 — watch `[test-mode:real] lane restricted to 5m`); (2) Task 4 15m lane + `series_id == "BTC-15m"` staging filter assert; (3) Task 5 prune dry-run gate; (4) Task 6 pm2 + `TEST_RUN_REPORT_<date>_MULTI_TF.md`; (5) optional: markets_log staging-drop hardening above.

> ## 🔁 LOOP STATUS (2026-09-07 ~08:40 UTC — multi-TF session: merge + tasks 1–2 done, 5m live validation FAILED on network outage, disk cleaned)
> Commits: `9cbca9f` (merge), `6e89569` (task1). Merged diverged branches: origin/master multi-TF lanes (`2519857`: per-(asset,tf) states, per-lane discovery 2s/5s/30s, per-lane Kaggle datasets + staging `kaggle_staging/{tf}/`, rolling-window prune, `check_and_roll_all`) + local loop2 hardening (no-skip discovery rule, backoff cap 5s, recovery probe, hermetic test env, finalize-wait). Hybrid kept: no-skip candidates + transport-abort fast retry (backoff 1.0s + `discovery_timeout` event); dropped pooled HTTP singleton (breaks monkeypatched fakes; per-call clients proven at 100%/99.93%). Fixed 2 stale `states["BTC"]` key uses in loop2 tests → `("BTC","5m")`.
> Task1: `resolution_backfill --all-lanes` (loops `cfg.timeframes` → `reupload_kaggle(timeframe=tf)`; default stays 5m-only), ecosystem.config.js header rewritten for single-process multi-TF (collector mem 800M→1G, backfill cron now `--reupload --all-lanes`), new `RUNBOOK.md` (machine spec, probe gate step 0, rollout 5m→15m→4h, log locations, prune safety).
> Task2 static: pytest **119/119 green** (114 + 5 restored loop2 discovery tests); probe re-run 2026-09-07: **5m/15m/4h ENABLE 7/7, 1h/1d still OFF**; `Collector(cfg)` constructs, 21 lanes (7×3), `timeframe_window_sizes()` matches yaml.
> Task3 5m live validation **FAILED — environmental, not code**: run 21:11 Sep 6 → 03:46 Sep 7, collector exit 0 BUT discovered windows 0, completeness 0%/clean 0%, `coverage_gap` 1. Total network outage overnight: DNS dead (`Name or service not known` on Gamma/CLOB/Kaggle), WS `timed out during opening handshake` ×222, Kaggle final upload DNS-failed (fail-closed, data retained). Merged discovery code verified working live AFTER the outage (finds BTC 5m market; DNS-failure path emits `discovery_timeout` + backoff 1.0 correctly in isolation). OPEN ANOMALY: zero `discovery_timeout`/`initial_discovery` events in parquet over ~6h of polling — expected thousands under outage; needs one investigation pass (starvation vs emit-path drop) before re-running validation. Kaggle 5m dataset delete at test start got 403 (tolerated, continued; final version push skipped by design since resolved=0).
> Ops: disk was **97% (318M free)** — cleaned to **79% (2.0G free)**: removed rotated `/var/log/syslog.1` (769M, spam), cleared `~/.cache` + `~/.npm` (~250M), journal 779M→148M. LEFT: `~/.local/share/opencode` 770M (user's tool, do not touch), `/var/log/syslog` active 153M. pm2 `cloud`/`opencode` online, uptime from ~08:29, 0 restarts (manual restart, not crash-loop; no reboot — up 3d16h). Collector never under pm2 on this box (Task 6 not done).
> NEXT: (1) investigate missing discovery_timeout events under outage; (2) re-run `python run_2x5min_test.py` (~25 min, needs network); (3) tasks 4–6 per `NEXT_AI_PROMPT_MULTI_TF_RUNNER.md` (15m lane ~40 min, prune dry-run gate, prod + report). Logs: `/tmp/opencode/test5m_stdout.log`, `test_run_20260906_211059.log`, analysis `data/test_analysis_final.json`.
